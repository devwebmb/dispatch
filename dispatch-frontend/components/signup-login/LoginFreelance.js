...
export default function LoginFreelance() {
  ...
  const onSubmit = (data) => {
    axios
      .post(`http://localhost:3080/api/user/freelance/login`, {
        email: data.email,
      ...
      .then((data) => {
        ...
        dispatch(
          setFreelanceData({
            ...
            email: data.data.data.freelance.email,
            firstname: data.data.data.freelance.firstname,
            lastname: data.data.data.freelance.lastname,
            societyname: "",
            freelanceId: data.data.data.freelance.id,
          })
        );
        router.push("/");
      })
